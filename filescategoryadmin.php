...
Access($_SESSION['EHID'], "filesadmin");
$groupsaccess = AccessGroups($_SESSION['EHID'], "filesadmin");
...
if(is_numeric($_GET['datatable'])) {
  ...
  <table>
    ...
    <?php
  $query = "SELECT FC_ID, Name, SortOrder FROM EH_Files_Categories WHERE Group_ID=$datatable Order By SortOrder";
...
<?php
  ...
  $query = "select FC_ID, SortOrder From EH_Files_Categories Where FC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $values = mysql_fetch_row($result);
  $curso = $values[1];
  $newso = $curso-1;
  $initialID = $values[0];
  $query = "select FC_ID From EH_Files_Categories Where SortOrder=$newso AND Group_ID=$group";
  $result = mysql_query($query, $mysql_link);
  $values = mysql_fetch_row($result);
  $swapID=$values[0];
  $query = "Update EH_Files_Categories Set SortOrder=$newso Where FC_ID=$initialID";
  $result = mysql_query($query, $mysql_link);
  $query = "Update EH_Files_Categories Set SortOrder=$curso Where FC_ID=$swapID";
  $result = mysql_query($query, $mysql_link);
  if($result)
    echo "<p>File Category moved up successfully!</p>\n";
  else
...
elseif($_GET['down']) {
  ...
  $result = mysql_query($query, $mysql_link);
  if($result)
    echo "<p>File Category moved down successfully!</p>\n";
  else
    echo "<p>ERROR! E-mail: $adminlink with details on what went wrong, with what data was attempted to be submitted</p>";
  }
elseif($_GET['edit']) {
  $id = mysql_real_escape_string($_GET['edit'], $mysql_link);
  $query = "SELECT FC_ID, Name FROM EH_Files_Categories WHERE FC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
...
elseif($_GET['edit1']) {
  ...
  $query = "UPDATE EH_Files_Categories Set Name='$name' WHERE FC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['add']) {
  $group = mysql_real_escape_string($_GET['group'], $mysql_link);
  $name = mysql_real_escape_string($_POST['name'], $mysql_link);
  $query = "SELECT MAX(SortOrder) FROM EH_Files_Categories WHERE Group_ID=$group";
  $result = mysql_query($query, $mysql_link);
  $rows = @mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  $query = "INSERT INTO EH_Files_Categories (Name, Group_ID, SortOrder) VALUES('$name', '$group', '$so')";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['del']) {
  ...
  $group = mysql_real_escape_string($_GET['group'], $mysql_link);
  $query = "SELECT SortOrder FROM EH_Files_Categories WHERE FC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  $query = "UPDATE EH_Files_Categories Set SortOrder=SortOrder-1 WHERE Group_ID=$group AND SortOrder>=$so";
  $result = mysql_query($query, $mysql_link);
  $query = "DELETE FROM EH_Files_Categories WHERE FC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
    echo "<p>File Category deleted successfully!</p>\n";
  else
    echo "<p>ERROR! E-mail: $adminlink with details on what went wrong, with what data was attempted to be submitted</p>";
  }
else {
  ...
  ?>
  <p>Emperor's Hammer File Category Administration</p>
  <p><a href="/menu.php">Return to the administration menu</a></p>
  <form name="selgroupform">
    <label for="selGroup">Select the Group to modify their File Categories</label>
...
?>
  ...
  <p>
      <a onClick="$('#add-form').dialog('open');" href="#">
        <span style="color:#6699CC;">Add New File Category</span>
    </a>
  </p>
  <div id="add-form" title="Add New File Category">
  ...
  <div id="editArea" title="Edit File Category">
    <form id="editForm" method="POST">
    </form>
  </div>
