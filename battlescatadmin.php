...
<?php
  $query = "SELECT BC_ID, Name, SortOrder FROM EH_Battles_Categories Order By SortOrder";
...
elseif($_GET['up']) {
  ...
  $query = "select BC_ID, SortOrder From EH_Battles_Categories Where BC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $values = mysql_fetch_row($result);
  $curso = $values[1];
  $newso = $curso-1;
  $initialID = $values[0];
  $query = "select BC_ID From EH_Battles_Categories Where SortOrder=$newso";
  $result = mysql_query($query, $mysql_link);
  $values = mysql_fetch_row($result);
  $swapID=$values[0];
  $query = "Update EH_Battles_Categories Set SortOrder=$newso Where BC_ID=$initialID";
  $result = mysql_query($query, $mysql_link);
  $query = "Update EH_Battles_Categories Set SortOrder=$curso Where BC_ID=$swapID";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['edit']) {
  ...
  $query = "SELECT BC_ID, Name, Abbr FROM EH_Battles_Categories WHERE BC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
...
elseif($_GET['edit1']) {
  ...
  $query = "UPDATE EH_Battles_Categories Set Name='$name', Abbr='$abbr' WHERE BC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['add']) {
  $name = mysql_real_escape_string($_POST['name'], $mysql_link);
  $abbr = mysql_real_escape_string($_POST['abbr'], $mysql_link);
  $query = "SELECT MAX(SortOrder) FROM EH_Battles_Categories";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  $query = "INSERT INTO EH_Battles_Categories (Name, Abbr, SortOrder) VALUES('$name', '$abbr', '$so')";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['del']) {
  ...
  $query = "SELECT SortOrder FROM EH_Battles_Categories WHERE BC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  $query = "UPDATE EH_Battles_Categories Set SortOrder=SortOrder-1 WHERE SortOrder>=$so";
  $result = mysql_query($query, $mysql_link);
  $query = "DELETE FROM EH_Battles_Categories WHERE BC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
  ...
  ?>
  <p>Emperor's Hammer Battle Categories Administration</p>
