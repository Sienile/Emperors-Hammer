...
Access($_SESSION['EHID'], "accesspageadmin");
...
<?php
  $query = "SELECT AP_ID, Name FROM EH_Access_Pages Order By Name";
...
elseif($_GET['edit']) {
  ...
  $query = "SELECT AP_ID, Name, needsAdmin FROM EH_Access_Pages WHERE AP_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  <form id="editForm" method="POST">
    <input type="hidden" name="id" value="<?=$id?>" />
      <table>
        ...
        <tr>
          <td><label for="admin">Requires Admin to assign: </label></td>
          <td>
              <input type="checkbox" name="admin" id="admin" value="1" <?=($values[2]==1) ? "checked=\"checked\"" : ""?>>
          </td>
        </tr>
      </table>
    </form>
...
elseif($_GET['edit1']) {
  ...
  if($admin)
    ...
    $admin=0;
  $query = "UPDATE EH_Access_Pages Set Name='$name', needsAdmin='$admin' WHERE AP_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['add']) {
  ...
  if($admin)
    ...
    $admin=0;
  $query = "INSERT INTO EH_Access_Pages (Name, needsAdmin) VALUES('$name', '$admin')";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['del']) {
  ...
  $query = "DELETE FROM EH_Access_Pages WHERE AP_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
  ...
  ?>
  <p>Emperor's Hammer Access Pages Administration</p>
  <p><a href="/menu.php">Return to the administration menu</a></p>
  <div id="message" style="color: green" ></div>
  <div id="response"></div>
  <p>
    <a onClick="$('#add-form').dialog('open');" href="#">
          <span style="color:#6699CC;">Add New Access Page</span>
      </a>
  </p>
  <div id="add-form" title="Add New Access Page">
