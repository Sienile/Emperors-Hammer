...
if(is_numeric($_GET['datatable'])) {
  ...
  <table>
    ...
    <?php
  $query = "SELECT EH_Competitions_Participants.CP_ID, EH_Members.Name, EH_Competitions_Participants.Score FROM EH_Competitions_Participants, EH_Members, EH_Competitions WHERE EH_Competitions_Participants.Comp_ID=$datatable AND EH_Competitions_Participants.Member_ID=EH_Members.Member_ID AND EH_Competitions.Admin_ID=".$_SESSION['EHID']." Order By EH_Competitions_Participants.Score, EH_Members.Name";
...
elseif($_GET['add']) {
  ...
  if($group) {
    ...
    echo "<p>No Competition Selected</p>";
    }
  }
elseif($_GET['del']) {
...
<?
    $query1 = "SELECT Comp_ID, Name FROM EH_Competitions WHERE Admin_ID=".$_SESSION['EHID']." AND Approved=1";
    $result1 = mysql_query($query1, $mysql_link);
    $rows1 = mysql_num_rows($result1);
    for($i=0; $i<$rows1; $i++) {
      $values1 = mysql_fetch_row($result1);
?>
    <option value="<?=$values1[0]?>"><?=$values1[1]?></option>
<?
      }
?>
  </select>
