...
<?php
  $query = "SELECT IC_ID, Name, SortOrder FROM EH_Images_Categories Order By SortOrder";
...
elseif($_GET['up']) {
  ...
  $query = "select IC_ID, SortOrder From EH_Images_Categories Where IC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $values = mysql_fetch_row($result);
  $curso = $values[1];
  $newso = $curso-1;
  $initialID = $values[0];
  $query = "select IC_ID From EH_Images_Categories Where SortOrder=$newso";
  $result = mysql_query($query, $mysql_link);
  $values = mysql_fetch_row($result);
  $swapID=$values[0];
  $query = "Update EH_Images_Categories Set SortOrder=$newso Where IC_ID=$initialID";
  $result = mysql_query($query, $mysql_link);
  $query = "Update EH_Images_Categories Set SortOrder=$curso Where IC_ID=$swapID";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['edit']) {
  ...
  $query = "SELECT IC_ID, Name, Display FROM EH_Images_Categories WHERE IC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
    ?>
<div id="editDiv" class="ajaxForm" title="Edit Image Category">
    ...
    <input type="hidden" name="id" value="<?=$id?>" />
      <table>
        ...
        <tr>
          ...
          <td>
              <input type="Checkbox" name="display" id="display" value="1" <?=($values[2]==1) ? "checked=\"checked\"" : ""?> >
          </td>
        </tr>
        <tr>
...
elseif($_GET['edit1']) {
  ...
  $avail = mysql_real_escape_string($_POST['display'], $mysql_link);
  if($avail)
    $avail=1;
  else
    $avail=0;
  $query = "UPDATE EH_Images_Categories Set Name='$name', Display='$avail' WHERE IC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['add']) {
  ...
  if($avail)
    ...
    $avail=0;
  $query = "SELECT MAX(SortOrder) FROM EH_Images_Categories";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  $query = "INSERT INTO EH_Images_Categories (Name, Display, SortOrder) VALUES('$name', '$avail', '$so')";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['del']) {
  ...
  $query = "SELECT SortOrder FROM EH_Images_Categories WHERE IC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  $query = "UPDATE EH_Images_Categories Set SortOrder=SortOrder-1 WHERE SortOrder>=$so";
  $result = mysql_query($query, $mysql_link);
  $query = "DELETE FROM EH_Images_Categories WHERE IC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
  ...
  ?>
  <p>Emperor's Hammer Images Categories Administration</p>
  <p><a href="/menu.php">Return to the administration menu</a></p>
  <div id="message" style="color: green" ></div>
  <div id="response"></div>
  <p>
      <a name="adddialog" onclick="$('#add').show()">
          <span style="color:#6699CC;">Add New Image Category</span>
      </a>
  </p>
  <div class="ajaxForm" style="display:none;" id="add" title="Add New Image Category">
  ...
  <div id="editdialog" title="Edit Image Category" >
