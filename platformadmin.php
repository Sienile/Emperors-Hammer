...
Access($_SESSION['EHID'], "platformadmin");
...
<?php
  $query = "SELECT Platform_ID, Name FROM EH_Platforms Order By Name";
...
elseif($_GET['edit']) {
  ...
  $query = "SELECT Platform_ID, Name, Abbr, FileExt FROM EH_Platforms WHERE Platform_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
    ?>
<div id="editDiv" class="ajaxForm" title="Edit Platform">
    ...
    <input type="hidden" name="id" value="<?=$id?>" />
      <table>
        ...
        <tr>
          <td><label for="ext">File Extension (used for processing BSFs): </label></td>
          <td><input type="text" name="ext" id="ext" value="<?=stripslashes($values[3])?>"></td>
        </tr>
        <tr>
...
elseif($_GET['edit1']) {
  ...
  $ext = mysql_real_escape_string($_POST['ext'], $mysql_link);
  $query = "UPDATE EH_Platforms Set Name='$name', Abbr='$abbr', FileExt='$ext' WHERE Platform_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['add']) {
  ...
  $abbr = mysql_real_escape_string($_POST['abbr'], $mysql_link);
  $ext = mysql_real_escape_string($_POST['ext'], $mysql_link);
  $query = "INSERT INTO EH_Platforms (Name, Abbr, FileExt) VALUES('$name', '$abbr', '$ext')";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['del']) {
  ...
  $query = "DELETE FROM EH_Members_Platforms WHERE Platform_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $query = "DELETE FROM EH_Platforms WHERE Platform_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
  ...
  ?>
  <p>Emperor's Hammer Platform Administration</p>
  <p><a href="menu.php">Return to the administration menu</a></p>
  <div id="message" style="color: green" ></div>
  <div id="response"></div>
  <p>
      <a name="adddialog" onclick="$('#add').show()">
          <span style="color:#6699CC;">Add New Platform</span>
      </a>
  </p>
  <div class="ajaxForm" style="display:none;" id="add" title="Add New Platform">
  <form id="addForm" method="POST" onSubmit="postAdd(); return false;">
    <table>
      ...
      <tr>
        <td><label for="ext">File Extension (For processing BSFs): </label></td>
  ...
  <div id="editdialog" title="Edit Platform" >
