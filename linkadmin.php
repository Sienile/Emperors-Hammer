...
<?php
  $query = "SELECT Link_ID, Name FROM EH_Links Order By LC_ID, Name";
...
elseif($_GET['edit']) {
  ...
  $query = "SELECT LC_ID, Name, URL, Description, LC_ID FROM EH_Links WHERE Link_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  <form id="editForm" method="POST">
    <input type="hidden" name="id" value="<?=$id?>" />
      <table>
        ...
        <tr>
          <td><label for="lcid">Category: </label></td>
          <td>
    <select name="lcid" id="lcid" >
    <?php
    $query1 = "SELECT LC_ID, Name FROM EH_Links_Categories Order By SortOrder";
...
elseif($_GET['edit1']) {
  ...
  $lcid = mysql_real_escape_string($_POST['lcid'], $mysql_link);
  $query = "UPDATE EH_Links Set Name='$name', URL='$url', Description='$desc', LC_ID='$lcid' WHERE Link_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['add']) {
  ...
  $desc = mysql_real_escape_string($_POST['desc'], $mysql_link);
  $lcid = mysql_real_escape_string($_POST['lcid'], $mysql_link);
  $query = "INSERT INTO EH_Links (Name, URL, Description, LC_ID) VALUES('$name', '$url', '$desc', '$lcid')";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['del']) {
  ...
  $query = "DELETE FROM EH_Links WHERE Link_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
  ...
  ?>
  <p>Emperor's Hammer Link Administration</p>
  <p><a href="/menu.php">Return to the administration menu</a></p>
  <div id="message" style="color: green" ></div>
  <div id="response"></div>
  <p>
      <a onClick="$('#add-form').dialog('open');" href="#">
          <span style="color:#6699CC;">Add New Link</span>
      </a>
  </p>
