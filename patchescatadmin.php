...
<?php
  $query = "SELECT PC_ID, Name, SortOrder FROM EH_Patches_Categories Order By SortOrder";
...
elseif($_GET['up']) {
  ...
  $query = "select PC_ID, SortOrder From EH_Patches_Categories Where PC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $values = mysql_fetch_row($result);
  $curso = $values[1];
  $newso = $curso-1;
  $initialID = $values[0];
  $query = "select PC_ID From EH_Patches_Categories Where SortOrder=$newso";
  $result = mysql_query($query, $mysql_link);
  $values = mysql_fetch_row($result);
  $swapID=$values[0];
  $query = "Update EH_Patches_Categories Set SortOrder=$newso Where PC_ID=$initialID";
  $result = mysql_query($query, $mysql_link);
  $query = "Update EH_Patches_Categories Set SortOrder=$curso Where PC_ID=$swapID";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['edit']) {
  ...
  $query = "SELECT PC_ID, Name, Description FROM EH_Patches_Categories WHERE PC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
    ?>
<div id="editDiv" class="ajaxForm" title="Edit Category">
...
elseif($_GET['edit1']) {
  ...
  $query = "UPDATE EH_Patches_Categories Set Name='$name', Description='$desc' WHERE PC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['add']) {
  $name = mysql_real_escape_string($_POST['name'], $mysql_link);
  $desc = mysql_real_escape_string($_POST['desc'], $mysql_link);
  $query = "SELECT MAX(SortOrder) FROM EH_Patches_Categories";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  $query = "INSERT INTO EH_Patches_Categories (Name, Description, SortOrder) VALUES('$name', '$desc', '$so')";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['del']) {
  ...
  $query = "SELECT SortOrder FROM EH_Patches_Categories WHERE PC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
  ...
  $query = "UPDATE EH_Patches_Categories Set SortOrder=SortOrder-1 WHERE SortOrder>=$so";
  $result = mysql_query($query, $mysql_link);
  $query = "DELETE FROM EH_Patches_Categories WHERE PC_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
  ...
  ?>
  <p>Emperor's Hammer Patch Categories Administration</p>
  ...
  <div class="ajaxForm" style="display:none;" id="add" title="Add New Category">
  ...
  <div id="editdialog" title="Edit Category" >
