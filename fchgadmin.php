...
<?php
  $query = "SELECT FCHG_ID, Name, Points FROM EH_FCHG Order By Points";
...
elseif($_GET['edit']) {
  ...
  $query = "SELECT FCHG_ID, Name, Abbr, Points, Image, StatsColor FROM EH_FCHG WHERE FCHG_ID=$id";
  $result = mysql_query($query, $mysql_link);
  $rows = mysql_num_rows($result);
  if($rows) {
    $values = mysql_fetch_row($result);
    ...
    <input type="hidden" name="id" value="<?=$id?>" />
      <table>
        ...
        <tr>
          ...
          <td><input type="text" name="points" id="points" value="<?=stripslashes($values[3])?>"></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td><input type="text" name="color" id="color" value="<?=stripslashes($values[5])?>"></td>
        </tr>
      </table>
    </form>
<?php
...
elseif($_GET['edit1']) {
  ...
  $query = "UPDATE EH_FCHG Set Name='$name', Abbr='$abbr', Points='$pts', Image='$img', StatsColor='$color' WHERE FCHG_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['add']) {
  ...
  $img = mysql_real_escape_string($_POST['img'], $mysql_link);
  $color = mysql_real_escape_string($_POST['color'], $mysql_link);
  $query = "INSERT INTO EH_FCHG (Name, Abbr, Points, Image, StatsColor) VALUES('$name', '$abbr', '$pts', '$img', '$color')";
  $result = mysql_query($query, $mysql_link);
  if($result)
...
elseif($_GET['del']) {
  ...
  $query = "DELETE FROM EH_FCHG WHERE FCHG_ID=$id";
  $result = mysql_query($query, $mysql_link);
  if($result)
    echo "<p>FCHG Rank deleted successfully!</p>\n";
  else
    echo "<p>ERROR! E-mail: $adminlink with details on what went wrong, with what data was attempted to be submitted</p>";
  }
else {
  ...
  ?>
  <p>Emperor's Hammer FCHG Rank Administration</p>
  <p><a href="/menu.php">Return to the administration menu</a></p>
  <div id="message" style="color: green" ></div>
  <div id="response"></div>
  <p>
      <a onClick="$('#add-form').dialog('open');" href="#">
          <span style="color:#6699CC;">Add New FCHG Rank</span>
      </a>
  </p>
  ...
  <div id="editArea" title="Edit FCHG Rank">
    <form id="editForm" method="POST">
    </form>
  </div>
